...
def sqrt(t, complex_dim=-1):
    sqrt_t_abs = torch.sqrt(absolute(t, complex_dim))
    sqrt_t_arg = arg(t, complex_dim) / 2
...
def abs_squared(t, complex_dim=-1):
    return get_real(t, complex_dim)**2 + get_imag(t, complex_dim)**2


def absolute(t, complex_dim=-1):
    return torch.sqrt(abs_squared(t, complex_dim=complex_dim))


def arg(t, complex_dim=-1):
...
def main():
    ...
    print(t1.int())
    print(t2.int())
    t1_mul_t2 = mul(t1, t2, complex_dim)
    print(t1_mul_t2.int())

    sqrt_t1_mul_t2 = sqrt(t1_mul_t2)
